...
{	
	...
	for (lightIter = lightlist->begin(); lightIter != lightlist->end(); lightIter++)
	{
		...
		{
			HitInfo newHit;
			if (scene.trace(newHit, sRay, 0.001, distance))								// Check for shadows
			{
				E *= (newHit.t < distance) ? 0 : 1;
			}
		}
	...
	if (m_reflectAmt*Rs > 0.0f && bounces < 3)											// Do two bounces of reflection rays.
	{
		...
		HitInfo newHit;
		if (scene.trace(newHit, rRay, 0.001, MIRO_TMAX))
		{
			Vector3 reflection = newHit.material->shade(rRay, newHit, scene);				
	...
	if (m_refractAmt*Ts > 0.0f)
	{
		...
		{		
			...
			HitInfo newHit;
			if (scene.trace(newHit, tRay, 0.001, MIRO_TMAX))
			{
				Vector3 refraction = newHit.material->shade(tRay, newHit, scene);				
...
Vector3 Blinn::shade(const Ray& ray, const HitInfo& hit, const Scene& scene) const
{
	...
	static Ray newRay;
#pragma omp threadprivate(newRay)

	MTRand_int32 irand(12345);
	MTRand drand;

	for (int i = 0; i < num_samples; i++)
	{
		Vector3 newVec = Vector3(drand(), drand(), drand()).normalized();
		float E = 1;
		float newVdotN = dot(theNormal, newVec);
		newVec = (newVdotN > 0.0) ? newVec : -newVec;
		newVdotN = fabsf(newVdotN);

		newRay.ox = P.x; newRay.oy = P.y; newRay.oz = P.z; newRay.ow = 1.0f;
		newRay.dx = newVec.x; newRay.dy = newVec.y; newRay.dz = newVec.z; newRay.dw = 0.0f;
		newRay.idx = 1.0f/newVec.x; newRay.idy = 1.0f/newVec.y; newRay.idz = 1.0f/newVec.z; newRay.idw = 0.0f;
		newRay.bounces_flags = IS_SHADOW_RAY;

		HitInfo sHit;
		if (scene.trace(sHit, newRay, 0.001, MIRO_TMAX))								// Check for shadows
		{
			E = 0;
		}

		//environment map lookup
		float theta = atan2(newVec.z, newVec.x) + PI;
		float phi = acos(newVec.y);
		float u = theta * 0.5 * piRecip;
		float v = 1.0 - (phi * piRecip);
		Vector4 tmp = m_envMap->getLookup3(u,v);
		Vector3 envColor = Vector3(tmp.x*num_samples_rec, tmp.y*num_samples_rec, tmp.z*num_samples_rec);
		Ld += m_kd*envColor*newVdotN*E;
	}
