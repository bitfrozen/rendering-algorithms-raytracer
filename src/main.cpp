...
void makeTrees(Objects* o, BVH* b, int n)
{
	for (int i = 0; i <= 345; i++)
	{
		...
		do 
		{
			x = Scene::getRand();
			z = Scene::getRand();
		} while (x*x + z*z > 1.f && sqrtf(x*x + z*z) < 0.25f && x < 0.f && z > 0.f);
		Matrix4x4 m = Matrix4x4();
		m.rotate(Scene::getRand()*360.f, 0, 1, 0);
		m.scale(Scene::getRand()*0.3+0.85, Scene::getRand()*0.3+0.85, Scene::getRand()*0.3+0.85);
		m.translate(x*x*4.f*125.f, Scene::getRand()*0.5f-0.5f, -z*z*4.f*125.f);
...
void makeFlowers(Objects* o, BVH* b, int n)
{
	for (int i = 0; i <= 400; i++)
	{
		...
		do 
		{
			x = Scene::getRand();
			z = Scene::getRand();
		} while (x*x + z*z > 1.f && sqrtf(x*x + z*z) < 0.5f && x < 0.f && z > 0.f);
		Matrix4x4 m = Matrix4x4();
		m.rotate(Scene::getRand()*360.f, 0, 1, 0);
		m *= Matrix4x4().rotateX(Scene::getRand()*20.f+10.f);
		m.scale(Scene::getRand()*0.2+0.9, Scene::getRand()*0.2+0.85, Scene::getRand()*0.2+0.9);
		m.translate(g_camera->eye().x+x*10.f, Scene::getRand()*0.025f-0.012f, g_camera->eye().z-z*10.f);
		ProxyObject* po = new ProxyObject(o, b, m);
		po->setDisplayNum(n);
		g_scene->addObject(po);
	}
...
void camera03Settings()
{
	// set up the camera
	g_scene->setBGColor(Vector3(0));
	g_camera->setEye(Vector3(22.962, 9.981, -44.288));
	g_camera->setViewDir(-0.442, 0.871, 0.212);
	g_camera->setUp(Vector3(0, 1, 0));
	g_camera->setFOV(50);
	g_camera->m_aperture = 0.0025f;
	g_camera->m_focusPlane = 10.f;
	g_camera->setShutterSpeed(0.0f);
}
...
void makeFinalScene()
{
	...
	//make a raw image from hdr file
	RawImage* hdrImage2 = new RawImage();
	hdrImage2->loadImage("Textures/sky.hdr");
	Texture* hdrTex2 = new Texture(hdrImage2);
...
// 	
	...
	Matrix4x4 T01m05 = Matrix4x4();
	T01m05.rotate(60, 0, 1, 0);
	T01m05.scale(0.85, 0.85, 0.85);
	T01m05.translate(80.558, 0, -70.04);
	...
	ProxyObject::setupMultiProxy(tree01Mesh, 2, tree01Mtl, tree01Os, tree01BVH);
 	...
 	addProxyObj(tree01Os, tree01BVH, T01m01, 10);
	addProxyObj(tree01Os, tree01BVH, T01m02, 10);
	addProxyObj(tree01Os, tree01BVH, T01m03, 10);
	addProxyObj(tree01Os, tree01BVH, T01m04, 10);
/*	addProxyObj(tree01Os, tree01BVH, T01m05, 10);*/

	//////////////////////////////////////////////////////////////////////////

	Matrix4x4 T03m01 = Matrix4x4();
	T03m01.rotate(0, 0, 1, 0);
	T03m01.scale(0.8,0.8,0.8);
	T03m01.translate(62.872, 0, -27.025);

	Matrix4x4 T03m02 = Matrix4x4();
	T03m02.rotate(0, 0, 1, 0);
	T03m02.scale(1, 1, 1);
	T03m02.translate(22.758, 0, -43.626);
	...
	tree03Leaves->load("Models/Final/tree03Leaves2.obj");

	BVH* tree03BVH = new BVH;
	Objects* tree03Os = new Objects;

	Material** tree03Mtl = new Material*[2];
	tree03Mtl[0] = Tree03BodyMtl;
	tree03Mtl[1] = tree03LeavesMtl;

	TriangleMesh** tree03Mesh = new TriangleMesh*[2];
	tree03Mesh[0] = Tree03Body;
	tree03Mesh[1] = tree03Leaves;

	ProxyObject::setupMultiProxy(tree03Mesh, 2, tree03Mtl, tree03Os, tree03BVH);

/*	makeTrees(tree03Os, tree03BVH, 200);*/
	addProxyObj(tree03Os, tree03BVH, T03m01, 50);
	addProxyObj(tree03Os, tree03BVH, T03m02, 50);
...
// 
// 	Blinn* grassMtl = new Blinn(Vector3(0.5));
// 	grassMtl->setSpecExp(15.0f);
// 	grassMtl->setSpecAmt(0.95);
// 	grassMtl->setTranslucency(0.25);
// 	grassMtl->setColorMap(grassTex);
// 
// 	TriangleMesh *grass = new TriangleMesh;
// 	grass->load("Models/testGrass2Hi.obj");
// 	BVH* b = new BVH;
// 	Objects* o = new Objects;
// 	ProxyObject::setupProxy(grass, grassMtl, o, b);
// 
